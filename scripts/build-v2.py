...
V2 = ROOT / 'v2'
projects = json.loads((V2 / 'projects.json').read_text())
...
(V2 / 'index.html').write_text(main)
header = re.search(r'<header class="header">.*?</header>', main, re.S).group()
header = header.replace('href="./"', 'href="/v2/"').replace('href="#', 'href="/v2/#')
footer = re.search(r'<footer class="footer wrap">.*?</footer>', main, re.S).group()
footer = footer.replace('href="./"', 'href="/v2/"')
urls = ['https://refactor.kz/v2/']
for p in projects:
    url = 'https://refactor.kz/v2/projects/' + p['slug'] + '/'
    urls.append(url)
