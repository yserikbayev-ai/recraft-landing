...
Версия `/v2/` размещена в `/opt/recraft-landing/v2/`. Для обновления загружайте только проверенное содержимое локального `v2/` в отдельный staging-каталог на сервере и заменяйте каталог версии. Конфигурация Caddy и перезапуск контейнера не требуются. Проверяйте публичный адрес и сохранность исходного `index.html`.
